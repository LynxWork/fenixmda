...
public class JSFCodeGenerator{
	 ...
	 *  
	 * ***/
	public String buildForm(Entity sysObject){
		StringBuilder resVal = new StringBuilder();
		

		resVal.append("<f:view>");
		resVal.append("<af:document id=\"d1\" title=\""+sysObject.getTitle()+"\">");
		resVal.append("<af:form id=\"f"+sysObject.getId()+"\">");
		 ...
		 * **************************************************************************************************/
		resVal.append("<af:panelFormLayout id=\"pfl"+sysObject.getId()+"\" rows=\"1\" maxColumns=\"2\">");
		resVal.append("<af:toolbar id=\"tl"+sysObject.getId()+"\">");
		resVal.append(buildFindButton(sysObject));
		resVal.append("</af:toolbar>");
		resVal.append("</af:panelFormLayout>");
		/****************************************************************************************************
		 *****************************************************************************************************/
		
		resVal.append("<af:spacer width=\"10\" height=\"20\" id=\"s"+sysObject.getId()+"\"/>");
		...
		resVal.append(buildOutputLabel(sysObject.getTitle(), "c"+sysObject.getId()));
		resVal.append(buildTable(sysObject));
	...
	public String buildFindButton(Entity sysObject){
		StringBuilder resVal = new StringBuilder();
		resVal.append(buildToolbarButton("Buscar"+sysObject.getTitle(),
				sysObject.getId().toString(),
				sysObject.getName()+".find"+sysObject.getName()+"List"));
		return resVal.toString();
	}
